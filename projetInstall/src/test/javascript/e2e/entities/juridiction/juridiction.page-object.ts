...
export default class JuridictionComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('juridiction-heading'));
...
export class JuridictionDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('audiencierApp.juridiction.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-juridiction'));
}
