...
export default class ContentieuxComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('contentieux-heading'));
...
export class ContentieuxDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('audiencierApp.contentieux.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-contentieux'));
}
