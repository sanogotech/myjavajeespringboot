...
export default class AudienceComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('audience-heading'));
...
export class AudienceDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('audiencierApp.audience.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-audience'));
}
